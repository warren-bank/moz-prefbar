...
function StartPrefBar(event) {
  ...
  // HACK: Cleanup view_toolbars_popup in SM >= 2.1
  ...
  // This should replaced with "nicer" code in future!
  if (goPrefBar.InSM()) {
    var menu = document.getElementById("view_toolbars_popup");
    if (menu) {
      menu.addEventListener("popupshowing", function(){
        var automenu = menu.getElementsByAttribute("id", "toggle_prefbar");
        var ourmenu = menu.getElementsByAttribute("id", "viewprefsbar");
        if (automenu.length == 1 && ourmenu.length == 1)
          ourmenu[0].parentNode.removeChild(ourmenu[0]);
      }, false);
    }
  }
